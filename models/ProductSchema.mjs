...
const productSchema = mongoose.Schema(
  {
    ...
    status: {
      ...
      enum: {
        ...
        message:
          "Status must be one of: active, inactive, discontinued, out-of-stock",
      },
      default: "active",
    },
    weight: {
...
productSchema.pre("save", function (next) {
  if (!this.sku) {
    this.sku = `PROD-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 9)
      .toUpperCase()}`;
  }
...
productSchema.statics.getByCategory = async function (
  categoryId,
  options = {}
) {
  const {
  ...
  if (includeSubcategories) {
    // Get all subcategories if needed
...
// Static method to get featured products by category
productSchema.statics.getFeaturedByCategory = async function (
  categoryId,
  limit = 10
) {
  return await this.find({
